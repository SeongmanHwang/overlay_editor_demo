...
- **로컬 데이터 용량 확인 기능 추가**
  - 앱 내에서 `%AppData%/SimpleOverlayEditor/` 하위 로컬 데이터(예: `aligned_cache/output/logs/session.json` 등)의 **폴더/파일별 용량 요약** 표시
  - UI: 홈 화면에 "로컬 데이터 용량 보기" 버튼 추가 (팝업/별도 창)
  - 성능: 대용량 폴더 스캔은 UI 프리즈 방지를 위해 **비동기 + 진행률 표시(ProgressRunner)** 적용
  - (선택) "폴더 열기", "캐시 정리" 버튼 제공
