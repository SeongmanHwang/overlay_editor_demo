...
6. **마킹 감지**: 채점 영역(ScoringArea)에서 마킹 여부 자동 감지
   - 그레이스케일 변환 후 평균 밝기 분석
   - 임계값 기반 마킹 판단 (기본값: 180)
   - 단일 이미지 또는 전체 이미지 일괄 감지
   - 감지 결과를 표로 표시 (영역별 마킹 여부, 평균 밝기)
7. **상태 저장**: AppData에 상태를 JSON으로 저장하여 재실행 시 복구
8. **결과 이미지 생성**: 오버레이가 적용된 이미지를 output 폴더에 저장
...
3. **템플릿 설정**: 
   - 오버레이 타입을 선택 (TimingMark 또는 ScoringArea)
   - "사각형 추가 모드" 토글 버튼을 활성화
   - 기본 크기(기본값: 30×30 픽셀)를 상단 도구바에서 조정 가능
   - 이미지 위를 클릭하면 클릭 위치를 중심으로 기본 크기의 사각형이 추가됩니다
   - "기본 템플릿 저장" 버튼으로 현재 템플릿을 저장할 수 있습니다
4. **사각형 편집**: 
   - 오버레이 목록에서 사각형을 선택
   - 오른쪽 패널에서 X, Y, Width, Height 값을 직접 수정
5. **마킹 감지**: 
   - 채점 영역(ScoringArea)을 설정한 후
   - 임계값을 조정 (기본값: 180, 0-255 범위)
   - "마킹 감지" 버튼으로 현재 이미지 감지 또는 "전체 감지"로 모든 이미지 일괄 감지
   - 오른쪽 패널의 "마킹 감지 결과"에서 각 영역별 마킹 여부 확인
6. **저장**: "저장" 버튼을 클릭하여 상태와 결과 이미지 저장
...
- 마킹 감지는 채점 영역(ScoringArea)의 평균 밝기를 기준으로 판단합니다
